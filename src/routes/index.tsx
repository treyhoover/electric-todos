...
export const Route = createFileRoute("/")({
  ...
  ssr: false,
  loader: async () => {
    await Promise.all([electricTodoCollection.preload(), electricConfigCollection.preload()]);
...
function ElectricPage() {
  ...
  const { data: todos } = useLiveQuery((q) =>
    q.from({ todo: electricTodoCollection }).orderBy(({ todo }) => todo.created_at, "asc"),
  );

  const { data: configData } = useLiveQuery((q) => q.from({ config: electricConfigCollection }));
