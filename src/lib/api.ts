...
export const api = {
  ...
  todos: {
    ...
    update: async (id: unknown, changes: Partial<SelectTodo>): Promise<{ todo: SelectTodo; txid: number }> => {
      const response = await fetch(`${API_BASE_URL}/todos/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    delete: async (id: unknown): Promise<{ success: boolean; txid: number }> => {
      const response = await fetch(`${API_BASE_URL}/todos/${id}`, {
        method: "DELETE",
      });
      if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
  ...
  config: {
    ...
    create: async (config: Partial<SelectConfig>): Promise<{ config: SelectConfig; txid: number }> => {
      const response = await fetch(`${API_BASE_URL}/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    update: async (id: number, changes: Partial<SelectConfig>): Promise<{ config: SelectConfig; txid: number }> => {
      const response = await fetch(`${API_BASE_URL}/config/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(changes),
